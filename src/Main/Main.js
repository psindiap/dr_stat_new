...
function Main() {
  ...
  const onSubmit = async (event) => {
    ...
    if (Number.isNaN(parseInt(inputs.hcm))) {
    ...
    const response2 = await fetch(
      `https://ninth-bonito-377309.el.r.appspot.com/db/insert`,
      {
        method: "POST",
        headers: {
          token: `Bearer ${jwt}`,
  ...
  return (
    <div className="bg-slate-300 flex flex-col justify-center items-center font-mono">
      <h1
        class="flex mb-4 text-3xl font-extrabold text-gray-900 dark:text-white md:text-3xl lg:text-3xl"
        style={{ padding: "10px" }}
      >
        <span class="break-all flex text-transparent bg-clip-text bg-gradient-to-r to-emerald-600 from-sky-600">
          DR DETECTION
        </span>
      </h1>
      ...
      >
        <div className="flex w-5/6 space-x-1  ">
          <div class="break-all shadow rounded-lg appearance-none w-1/3 py-2 px-3 text-black-700 leading-tight focus:outline-none focus:shadow-outline">
            Age:
          </div>
          <input
            class="break-all shadow rounded-lg appearance-none border rounded w-2/3 py-2 px-3 text-black-700 leading-tight focus:outline-none focus:shadow-outline "
            id="age"
            type="text"
            placeholder="Age must be between 0-100 years"
            value={inputs.age}
            onChange={(e) => {
              e.preventDefault();
        ...
        <div className="flex w-5/6 space-x-1">
          ...
          <select
            id="gender"
            class="break-all flex border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-2/3 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
            value={inputs.gender}
            onChange={(e) => {
              e.preventDefault();
        ...
        {/* <div className="flex w-5/6 space-x-1">
          <label
            ...
            class="break-all flex shadow rounded-lg appearance-none w-1/3 py-2 px-3 text-black-700 leading-tight focus:outline-none focus:shadow-outline"
          >
            Glycosylated Haemoglobin Status:
          </label>
          <select
            id="ghs"
            class="break-all flex border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-2/3 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
          ...
          >
            <option selected>Choose an option</option>
            <option value="0">{"No Diabetes (<5.7%)"}</option>
      ...
      <div style={{ padding: "20px" }}>
        <button
          onClick={onSubmit}
          class="break-all flex bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 rounded"
        >
          Predict
        </button>
      </div>
      ...
      <h1
        ...
        style={{ padding: "25px" }}
      >
        <span class="break-all flex text-transparent bg-clip-text bg-gradient-to-r to-pink-600 from-purple-600">
          RESULT:{" "}
        </span>
        {result === 1 ? (
          <span class="break-all flex text-red-600">DR POSITIVE</span>
        ) : result === 0 ? (
          <span class="break-all flex text-green-600">DR NEGATIVE</span>
        ) : (
          ""
        )}
      </h1>
